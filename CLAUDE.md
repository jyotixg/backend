...
18. Deployment — ✅ done (LIVE on Render + Neon Postgres) 🎉 **ROADMAP COMPLETE**

## 🎉 Roadmap complete
All 18 steps done. The API is deployed live on Render (Node app) + Neon (cloud Postgres). Live URL: `https://auth-api-76pc.onrender.com`. If the user wants more, natural enhancements beyond the roadmap: real email provider (Resend/SendGrid/SES) instead of Ethereal, a `BASE_URL` env var for email links, DB-stored/revocable refresh tokens, and a frontend.
...
- **Day 18:** Deployment — went LIVE. Hosted Postgres on **Neon**; deployed the Node app to **Render** (build `npm install && npx prisma generate && npx prisma migrate deploy`, start `npm start`, env vars `DATABASE_URL`/`JWT_SECRET`/`JWT_REFRESH_SECRET` in dashboard; added `postinstall: prisma generate` + moved `prisma` to deps). Debugged: unpushed local changes (host builds GitHub not laptop), `//register` trailing-slash bug, registration 500-but-user-saved (email side effect → wrapped `sendEmail` in its own try/catch), Ethereal SMTP `ETIMEDOUT` on Render (added connection timeouts; Ethereal is dev-only — verify cloud users via Neon SQL). Live at `https://auth-api-76pc.onrender.com`. Notes: `notes/18-deployment.md`.
